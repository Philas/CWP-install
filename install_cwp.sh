...
find /usr/local/php/ /opt/alt/php-fpm*/ -name "php.ini" | xargs sed -i 's/^upload_max_filesize.*/upload_max_filesize = 32M/g'
find /usr/local/php/ /opt/alt/php-fpm*/ -name "php.ini" | xargs sed -i 's/^post_max_size.*/post_max_size = 32M/g'
find /usr/local/php/ /opt/alt/php-fpm*/ -name "php.ini" | xargs sed -i 's/^date.timezone.*/date.timezone = "Europe\/Vilnius"/g'
...
find /usr/local/php/ /opt/alt/php-fpm*/ -name "php.ini" | xargs sed -i 's/^display_errors.*/display_errors = Off/g'
...
if [ -f /usr/share/zoneinfo/Europe/Vilnius ]; then
